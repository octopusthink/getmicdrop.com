...
Based on current user feedback, API limitations in macOS, and the potential of Apple nixing the Touch Bar altogether, we don't have any plans to support the Touch Bar ourselves.

That said, if you're a fan of the Touch Bar, there's a workaround that may do the trick: use [BetterTouchTool](https://folivora.ai) to configure a Touch Bar button that will activate your Mic Drop keyboard shortcut, and you should be good to go!

Bonus: if you want to query the mute state of Mic Drop to change the icon based on whether you're muted or not, you can try using this command to get the mute status: `defaults read com.octopusthink.Mic-Drop devicesMuted`.
...
If you can map a button on your mouse, headset, etc. to your Mic Drop keyboard shortcut directly then you're all set: several users have used the Logi Options software with MX series mice to do this, allowing a button on their mouse to mute/unmute Mic Drop.

If you need to use software to map a button press/key to your Mic Drop keyboard shortcut—we recommend [BetterTouchTool](https://folivora.ai).

Cats, as a rule, will only press keys that you _don't_ want pressed. 😼
...
## My microphone/audio input device isn't supported. Is there a workaround?

Indeed there is! Many users have had success [using Loopback to create a virtual audio device](/help/using-loopback) and muting that device with Mic Drop. Mic Drop is compatible with Loopback audio devices and in our experience this has worked quite well.
