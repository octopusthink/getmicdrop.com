...
**We fixed this bug** in Mic Drop 1.4. If you just want your headphones to work again, <DownloadLink>download the latest version of Mic Drop</DownloadLink> and you're golden.
...
We detected the issue in August and filed a [bug report](https://twitter.com/octopusthink/status/1431689504126078977) with Apple. They haven't responded or resolved the issue, so it's not clear when it will be fixed.
...
Because Mic Drop will use the built-in microphone instead of a Bluetooth microphone, this can cause issues when you're using clamshell mode and Bluetooth. We're working on a fix to ignore the built-in microphone when in chamshell mode, but for now: if you use clamshell mode please select another wired microphone (like the one built into your webcam) for your microphone audio to work.
